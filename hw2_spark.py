...
def func_question1_2():

    path_books = "/MSAN_USF/courses_spring/630_AdvMachinelearning/code/data/BX-CSV-Dump/"
...
def naive_bayes_basic():
    ...
    test_Npath = te_folder+"neg/*.txt"
    test_Ppath = te_folder+ "pos/*.txt"
    # test_Npath = "/MSAN_USF/courses_spring/630_AdvMachinelearning/code/data/nb_review_classifier/test/neg/*.txt"
    # test_Ppath = "/MSAN_USF/courses_spring/630_AdvMachinelearning/code/data/nb_review_classifier/test/pos/*.txt"
    ...
    for doc_n in test_n:
        ...
        doc_n_rdd = sc.parallelize(new_doc).map(lambda x: removeStopWords(x))
    ...
    for doc_p in test_p:
        doc_p_rdd = sc.parallelize(doc_p)
    ...
    sum_matching = 0.0
    total_items = dict_files.keys().count()
